...
public class UtilString {
   ...
   public static int numDigits(String text) {
    ...
    public static boolean esSubstring(String substring, String text, boolean esEstricte) {
    if (esEstricte) {
        ...
        int limitCerca = text.length() - substring.length();
        for (int i = 0; i <= limitCerca; i++) {

            String conte = intervalString(text, i, i + substring.length() - 1);
            
            if (conte.equals(substring)) {
                return true;
            }
        }
        return false;
        ...
        String substringBrut = normalitzaText(substring);
        String textBrut = normalitzaText(text);
        
        
        String substringNet = normalitzaBlancs(substringBrut);
        String textNet = normalitzaBlancs(textBrut);

        if (substringNet.length() > textNet.length()) { 
          return false;
          }
          
         if (substringNet.length() == 0)  { 
         return true;
         }
     

        int limitCercaNet = textNet.length() - substringNet.length();
        for (int i = 0; i <= limitCercaNet; i++) {
            
            String conte = intervalString(textNet, i, i + substringNet.length() - 1);
            
           
            if (conte.equalsIgnoreCase(substringNet)) {
                return true;
            }
        }
        return false;
   ...
   public static boolean esSubstring(String substring, String text) {
      return esSubstring(substring, text, true);
   }
